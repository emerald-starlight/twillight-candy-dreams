...
function unflatten(target, opts) {
  ...
  function isEmpty(val) {
    ...
    const isArray = type2 === "[object Array]";
  ...
  target = Object.keys(target).reduce(function(result2, key2) {
    const type2 = Object.prototype.toString.call(target[key2]);
    const isObject2 = type2 === "[object Object]" || type2 === "[object Array]";
    if (!isObject2 || isEmpty(target[key2])) {
...
function stringifyFrontMatter(data, content = "") {
  ...
  return [
    ...
    js_yaml_default.dump(data, { lineWidth: -1 }).trim(),
    FRONTMATTER_DELIMITER_DEFAULT,
    content
  ].join("\n");
}
...
function parseFrontMatter(content) {
  ...
  if (content.startsWith(FRONTMATTER_DELIMITER_DEFAULT)) {
    ...
    if (idx !== -1) {
      ...
      if (frontmatter) {
        data = js_yaml_default.load(frontmatter);
...
var toMarkdown = (opts = {}) => {
  const experimentalAutoUnwrap = (node) => {
    var _a, _b;
    if ((_a = opts == null ? void 0 : opts.experimental) == null ? void 0 : _a.autoUnwrap) {
      if ((_b = node.mdc) == null ? void 0 : _b.unwrapped) {
        node.children = [
          {
...
${content(node, context)}`.trim();
  ...
  function containerComponent(node, _, context) {
    ...
    if ((value + attributesText).length > 80 || Object.keys(fmAttributes).length > 0 || ((_a = node.children) == null ? void 0 : _a.some((child) => child.type === "componentContainerSection"))) {
    ...
    if (Object.keys(fmAttributes).length > 0) {
      const attrs = Object.entries(fmAttributes).sort(([key1], [key2]) => key1.localeCompare(key2)).reduce((acc, [key2, value2]) => {
        if (key2 == null ? void 0 : key2.startsWith(":")) {
          try {
      ...
      const fm = ((_b = opts == null ? void 0 : opts.experimental) == null ? void 0 : _b.componentCodeBlockYamlProps) ? stringifyCodeBlockProps(attrs) : stringifyFrontMatter(attrs);
      value += "\n" + fm.trim();
    }
  ...
  function attributes2(node, context) {
    ...
    const attrs = Object.fromEntries(
      Object.entries(node.attributes || {}).sort(([key1], [key22]) => key1.localeCompare(key22))
    );
    const values = [];
    let id;
    let classesFull = "";
    ...
    let index2;
    for (key2 in attrs) {
      if (own2.call(attrs, key2) && attrs[key2] != null) {
        value = String(attrs[key2]);
        if (key2 === "id") {
    ...
    function quoted(key22, value2) {
      return key22 + "=" + quote + stringifyEntitiesLight(value2, { subset }) + quote;
    }
  }
  ...
  return {
    ...
    handlers: {
      ...
      link: (node, _, state, info) => {
        return handle.link(node, _, state, info) + attributes2(node, state);
      },
...
var fromMarkdown = (opts = {}) => {
  ...
  const experimentalAutoUnwrap = (node) => {
    var _a;
    if (((_a = opts.experimental) == null ? void 0 : _a.autoUnwrap) && NON_UNWRAPPABLE_TYPES.includes(node.type)) {
  ...
  function enterBindingContent(token) {
    ...
    this.enter({
      ...
      name: "binding",
      attributes: {
        value: (_a = values == null ? void 0 : values[1]) == null ? void 0 : _a.trim(),
        defaultValue: values == null ? void 0 : values[2]
      }
    }, token);
  }
  ...
  function exitContainer(token) {
    ...
    if (container.children.length > 1) {
      ...
      container.rawData = dataSection == null ? void 0 : dataSection.rawData;
    }
    ...
    container.children = container.children.flatMap((child) => {
      ...
      if (child.name === "default" || !child.name) {
        if ((_a = child.mdc) == null ? void 0 : _a.unwrapped) {
          container.mdc = container.mdc || {};
          container.mdc.unwrapped = (_b = child.mdc) == null ? void 0 : _b.unwrapped;
        }
  ...
  function exitContainerSectionTitle(token) {
    var _a;
    this.stack[this.stack.length - 1].name = (_a = this.sliceSerialize(token)) == null ? void 0 : _a.trim();
  }
  ...
  function enterToken(type2, token) {
  ...
  function exitAttributeValue(token) {
    const attributes2 = this.data.componentAttributes;
    attributes2[attributes2.length - 1][1] = parseEntities(this.sliceSerialize(token));
  }
  ...
  function exitAttributes() {
    ...
    if (stackTop.type !== "textComponent" || stackTop.name === "span") {
      while (!((_a = stackTop.position) == null ? void 0 : _a.end) && ((_b = stackTop.children) == null ? void 0 : _b.length) > 0) {
...
function createLabel(effects, ok, nok, type2, markerType, stringType, disallowEol) {
  let size = 0;
  let balance = 0;
  return start;
  function start(code) {
...
function createAttributes(effects, ok, nok, attributesType, attributesMarkerType, attributeType, attributeIdType, attributeClassType, attributeNameType, attributeInitializerType, attributeValueLiteralType, attributeValueType, attributeValueMarker, attributeValueData, disallowEol) {
  ...
  function shortcutStart(code) {
    effects.enter(attributeType);
    effects.enter(type2);
    effects.enter(type2 + "Marker");
    effects.consume(code);
    effects.exit(type2 + "Marker");
...
var index = function(opts = {}) {
  ...
  if ((_a = opts == null ? void 0 : opts.components) == null ? void 0 : _a.length) {
...
function getNodeData(node) {
  ...
  if (((_a = node.children) == null ? void 0 : _a.length) && node.children[0].type === "code" && node.children[0].lang === "yaml" && node.children[0].meta === "[props]") {
    const yaml = node.children[0].value;
    const { data } = parseFrontMatter(toFrontMatter(yaml));
    node.rawData = yaml + "\n---";
...
*/
//# sourceMappingURL=remark-mdc.js.map
